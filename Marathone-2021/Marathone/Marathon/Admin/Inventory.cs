...
namespace Marathon.Admin
{
    public partial class Inventory : MetroForm
    {
        ...
        public void updateCounts()
        {
            ...
            {
                string sql = $"SELECT Count FROM RacеKitОption WHERE RaceKitOptionId='{Ids[i]}'";
                MySqlCommand sqlCommand = new MySqlCommand(sql, Program.connection);
        ...
        private void fillPanel()
        {
            ...
            try
            {
                ...
                sql = "SELECT (SELECT Count FROM RacеKitОption WHERE RaceKitOptionId='A')+(SELECT Count FROM RaceKitOption WHERE RaceKitOptionId='B')+(SELECT Count FROM RaceKitOption WHERE RaceKitOptionId='C') as Remains";
                sqlCommand = new MySqlCommand(sql, Program.connection);
                sqlDataReader = sqlCommand.ExecuteReader();
                sqlDataReader.Read();
        ...
        private void metroButtonEntrance_Click(object sender, EventArgs e)
        {
            InventoryReceipt form = new InventoryReceipt();
            form.Show();
            this.checkCancelButton = true;
            this.Close();
        }
