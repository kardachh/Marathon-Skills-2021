    ...
    public partial class EditUser : MetroForm
    {
        ...
        private void metroButton1_Click(object sender, EventArgs e)
        {
            if (metroTextBox2.Text == metroTextBox3.Text)
            {
                ...
                {
                    role = "R";
                    MySqlCommand command = new MySqlCommand("UPDATE user SET Password = @password, FirstName = @firstname, LastName = @lastname, RoleId = @role WHERE Email = @login", Program.connection);
                    ...
                    MySqlCommand command1 = new MySqlCommand("UPDATE runner SET Gender = @gender,DateOfBirth = @dateOfBirth,CountryCode = @countryCode WHERE Email = @login", Program.connection);
