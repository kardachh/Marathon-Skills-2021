...
namespace Marathon.Runner
{
    public partial class RunnerEventRegister : MetroForm
    {
        ...
        public RunnerEventRegister(string data)
        {
            ...
            timer1.Start();
            MySqlCommand roleCommand = new MySqlCommand("SELECT RunnerId FROM Runnеr WHERE Email = @login", Program.connection);
            Program.connection.Open();
            roleCommand.Parameters.AddWithValue("@login", data);
            roleCommand.Prepare();
            var value = roleCommand.ExecuteScalar();
            login = value.ToString();
            MySqlCommand newcommand = new MySqlCommand("SELECT DISTINCT CharityName FROM Chаritу", Program.connection);
        ...
        private void metroButton1_Click(object sender, EventArgs e)
        {
            ...
            Program.connection.Open();
            MySqlCommand command = new MySqlCommand("INSERT INTO Registration (RunnerId, RegistrationDateTime, RaceKitOptionId, RegistrationStatusId, Cost, CharityId, SponsorshipTarget) VALUES (@RunnerId, @RegistrationDateTime, @RaceKitOptionId, @RegistrationStatusId, @Cost, @CharityId, @SponsorshipTarget)", Program.connection);
