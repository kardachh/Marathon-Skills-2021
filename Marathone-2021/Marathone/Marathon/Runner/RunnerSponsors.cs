    ...
    public partial class RunnerSponsors : MetroForm
    {
        ...
        public RunnerSponsors(string data)
        {
            ...
            try
            {
                ...
                MySqlCommand roleCommand1 = new MySqlCommand("SELECT RunnerId FROM Runnеr WHERE Email = @login", Program.connection);
                ...
                MySqlCommand roleCommand = new MySqlCommand("SELECT RegistrationId FROM Registration WHERE RunnerId = @login", Program.connection);
                roleCommand.Parameters.AddWithValue("@login", login);
                roleCommand.Prepare();
                var value1 = roleCommand.ExecuteScalar();
                string id = value1.ToString();
                MySqlDataAdapter da = new MySqlDataAdapter("SELECT SponsorName, Amount FROM Sponsоrship WHERE RegistrationId =\"" + id + "\"", Program.connection);
        ...
        private void RunnerSponsors_Load(object sender, EventArgs e)
        {
